...
export function Dashboard() {
  ...
  const isCurrentMonth = useMemo(() => {
    const n = new Date();
    return n.getFullYear() === selectedMonth.getFullYear() && n.getMonth() === selectedMonth.getMonth();
  }, [selectedMonth]);
  ...
  const pieData = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([categoryId, value]) => {
      ...
      return { categoryId, name: cat?.name ?? "Other", value, color: `hsl(${cat?.color ?? "186 41% 38%"})` };
    });
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          >
            {isCurrentMonth ? "This month at a glance" : "At a glance"}
          </motion.h1>
        ...
        {cards.map((c, i) => {
          ...
          return (
            <motion.div
              ...
              role="button"
              tabIndex={0}
              onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") drillDown(); }}
              className={`rounded-2xl p-5 shadow-elevated card-hover cursor-pointer ${c.accent}`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        >
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Last 6 months ending {selectedMonth.toLocaleDateString("en-US", { month: "short", year: "numeric" })}
          </p>
          <div className="h-72">
        ...
        >
          ...
          <p className="text-sm text-muted-foreground mb-4">
            By category — {selectedMonth.toLocaleDateString("en-US", { month: "long", year: "numeric" })}
          </p>
          {pieData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    outerRadius={95}
                    paddingAngle={2}
                    onClick={(d: any) => drillDown(d?.categoryId)}
                    cursor="pointer"
                  >
